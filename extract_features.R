extract_features <- function(fs,metadata,dir){
  ...
  ############## All metrics together (only significant) #############
  
  sig=which(Kstest_p<0.05)
  
  if (length(sig)>0){
    
    dir.create(paste0(dir,"/All_sig"), showWarnings = FALSE)
    Mom=Mom[sig]
    
    # Compute KStest pvals and print barplot
    Kstest_p=apply(Mom, 2, function(x) ks.test(x[metadata$Status=="R"],x[metadata$Status=="NR"])$p)

    pdf(paste0(dir,"/All_sig","/KStest_pvals.pdf"),width=20,height=5)
    print(ggplot(data.frame(Pval=Kstest_p,Marker=colnames(Mom)),aes(Marker,Pval))+geom_col(color="black")+geom_hline(yintercept=0.05, linetype="dashed", color = "red")+theme_bw()+theme(text = element_text(size = 20)))
    dev.off()
    ...
    pdf(paste0(dir,"/All_sig","/Boxplot.pdf"),width=20,height=5)
    print(ggplot(meltData, aes(x=variable, y=value,fill=Status))+geom_boxplot()+scale_color_manual(values=c("#00BFC4","#F8766D")))
    dev.off() 
    ...
    pdf(paste0(dir,"/All_sig","/Corr.pdf"),width=15,height=15)
    corrplot(correlations$r, p.mat = correlations$P,type="lower",insig="label_sig")
    dev.off()
    
    # Save values
    Mom$Status=metadata$Status
    write_xlsx(Mom,paste0(dir,"/All_sig","/Values.xlsx"))
