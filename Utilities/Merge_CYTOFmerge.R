Merge_CYTOFmerge <- function(patient,Qnorm=TRUE) {
  ...
  for (i in sample(1:length(files))){ # for each file
    ...
    for (j in 1:length(left[[i]])){ # for each missing marker
      ...
      if (paste(reference[[j+1]],collapse = "")==paste(reference[[j]],collapse = "")){ # Copy from previous iteration
        ...
        Merged_updated[cells_unmeasured,marker]=apply(matrix(Merged_file[cells_measured[nn], marker],ncol=50),1,median)
        ...
        # Compute neighbors
        nn = get.knnx(df_measured,df_unmeasured,k=50,algorithm="kd_tree")$nn.index
