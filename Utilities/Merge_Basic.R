Merge_Basic <- function(patient,Qnorm=TRUE) {
  ...
  for (i in sample(1:length(files))){ # for each file
    ...
    for (j in 1:length(left[[i]])){ # for each missing marker
      ...
      if (paste(reference[[j+1]],collapse = "")==paste(reference[[j]],collapse = "")){ # Copy from previous iteration
        ...
        Merged_updated[cells_unmeasured,marker]=Merged_file[cells_measured[nn],marker]
        ...
        # Compute neighbors
        nn = get.knnx(df_measured,df_unmeasured,k=1,algorithm="kd_tree")$nn.index
